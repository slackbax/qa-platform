...
class Accidente {
	...
	public function get($id): stdClass
	{
		...
		$stmt = $db->Prepare("SELECT *
                                    ...
                                    LEFT JOIN uc_profesion up on a.pro_id = up.pro_id
		...
		$obj->pro_nombre = utf8_encode($row['pro_nombre']);
		$obj->us_id = $row['us_id'];
	 ...
	 * @return array
	 */
	public function set($ser, $us, $us_mod, $esta, $prof, $fecha, $fecha_acc, $nombres, $ap, $am, $lugar, $desc, $vacuna, $tiempo, $ficha, $medico, $fuente, $aviso, $diat, $seguim, $aten, $serol, $trat, $prot, $db): array
	{
		if (is_null($db)):
			$db = new myDBC();
		endif;
		...
		try {
			$stmt = $db->Prepare("INSERT INTO uc_acc_laboral (ser_id, us_id, us_mod_id, esta_id, pro_id, acl_fecha, acl_fecha_acc, acl_nombres, acl_ap, acl_am, acl_lugar, acl_descripcion, acl_vacuna, 
			...
			$trat = ($trat == '') ? null : $trat;
			$prot = ($prot == '') ? null : $prot;
			$bind = $stmt->bind_param("iiiiisssssssisssiiisissi", $ser, $us, $us_mod, $esta, $prof, $fecha, $fecha_acc, $nombres, $ap, $am, $lugar, $desc, $vacuna,
	 ...
	 * @return array
	 */
	public function mod($id, $ser, $us_mod, $esta, $prof, $fecha, $fecha_acc, $nombres, $ap, $am, $lugar, $desc, $vacuna, $tiempo, $ficha, $medico, $fuente, $aviso, $diat, $seguim, $aten, $serol, $trat, $prot, $db): array
	{
		if (is_null($db)):
			$db = new myDBC();
		endif;
		...
		try {
			$stmt = $db->Prepare("UPDATE uc_acc_laboral SET ser_id = ?, us_mod_id = ?, esta_id = ?, pro_id = ?, acl_fecha = ?, acl_fecha_acc = ?, acl_nombres = ?, acl_ap = ?, acl_am = ?, acl_lugar = ?, acl_descripcion = ?, acl_vacuna = ?, 
			...
			$trat = ($trat == '') ? null : $trat;
			$prot = ($prot == '') ? null : $prot;
			$bind = $stmt->bind_param("iiiisssssssssssssssssssi", $ser, $us_mod, $esta, $prof, $fecha, $fecha_acc, $nombres, $ap, $am, $lugar, $desc, $vacuna,
